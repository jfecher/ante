...
pub fn call_builtin<'ast>(builtin: &'ast Builtin, context: &mut Context<'ast>, builder: &mut FunctionBuilder) -> Value {
    let mut value = |ast: &'ast Atom| ast.eval_single(context, builder);
...
fn transmute<'ast>(
    context: &mut Context<'ast>, param: &'ast Atom, typ: &crate::hir::Type, builder: &mut FunctionBuilder,
) -> Value {
...
fn deref<'ast>(context: &mut Context<'ast>, typ: &crate::hir::Type, addr: &'ast Atom, builder: &mut FunctionBuilder) -> Value {
    let addr = addr.eval_single(context, builder);
    context.load_value(typ, addr, &mut 0, builder)
}

fn stack_alloc<'ast>(param1: &'ast Atom, context: &mut Context<'ast>, builder: &mut FunctionBuilder) -> CraneliftValue {
